...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    return np.maximum(0, 1 - label*prediction)
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    hinge_losses = np.maximum(0, 1 - labels * predictions)
    average_loss = np.mean(hinge_losses)
    return average_loss
...
def perceptron_single_step_update(
    ...
    prediction = label*(np.dot(current_theta, feature_vector) + current_theta_0)
    if prediction <= 0:
...
def perceptron(feature_matrix, labels, T):
    ...
    current_theta = np.zeros(feature_dim)
    current_theta_0 = 0
...
def average_perceptron(feature_matrix, labels, T):
    ...
    current_theta = np.zeros(feature_dim)
    current_theta_0 = 0
    sum_theta = np.zeros(feature_dim)
    sum_theta_0 = 0
...
def pegasos(feature_matrix, labels, T, L):
    ...
    for t in range(T):
        ...
        for i in get_order(nsamples):
            eta_t = 1 / np.sqrt(counter)
            current_theta, current_theta_0 = pegasos_single_step_update(feature_matrix[i], labels[i], L, eta_t, current_theta, current_theta_0)
            counter += 1
...
def classify(feature_matrix, theta, theta_0):
    ...
    """
    # Your code here
    return np.maximum(0, feature_matrix@theta + theta_0 > 0.0001)*2-1
...
def classifier_accuracy(
    ...
    predictions_on_training_set = classify(train_feature_matrix, theta, theta0)
    predictions_on_validation_set = classify(val_feature_matrix, theta, theta0)
    return accuracy(predictions_on_training_set, train_labels), accuracy(predictions_on_validation_set, val_labels)
