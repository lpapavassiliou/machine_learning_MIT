...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    if random.random() < epsilon:
        ...
        # get the coordinate of the maximum element of Q(state1, state2)
...
def tabular_q_learning(q_func, current_state_1, current_state_2, action_index,
    ...
    Args:
        q_func (np.ndarray): current Q-function (state1,state2,action,object)
    ...
    else:          # consider the reward and the value from the next state until the end
        q_func[current_state_1, current_state_2, action_index,
            object_index] = (1-ALPHA)*q_func[current_state_1, current_state_2, action_index,
                                object_index] + ALPHA*(reward + GAMMA*np.max(q_func[next_state_1, next_state_2, :, :]))
